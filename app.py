...
import pandas as pd
from assets import read_data, preprocess_and_print, pelabelan, tf_idf, proses_pca, fold, eval_akhir, fold_no
...
with st.sidebar:
    selected = option_menu( 
        ...
        options=["Home", "Dataset", "Preprocessing", "TF-IDF", "PCA", "Evaluasi"],
        icons=["house", "book", "radioactive", "bezier2", "bounding-box", "ubuntu"],
        menu_icon="menu-up",
        default_index=0,
    )
...
if selected == "Home":
    st.title("Home")
    st.subheader("Model Naive Bayes dan PCA untuk Data Sentimen Masyarakat (Reaktivasi Kereta Api Madura)")
...
if selected == "Evaluasi":
    ...
    evaluasi = fold(st.session_state.data_tf, st.session_state.data, n, st.session_state.h_pca)
