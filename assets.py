...
def fold(a, b, n, c):
    ...
    for fold, (train_index, test_index) in enumerate(kf.split(c), 1):
        ...
        precision = round(precision_score(y_test_fold, y_pred_fold, average='weighted', zero_division=1) * 100, 2)
        recall = round(recall_score(y_test_fold, y_pred_fold, average='weighted', zero_division=1) * 100, 2)
        f1 = round(f1_score(y_test_fold, y_pred_fold, average='weighted', zero_division=1) * 100, 2)
        results.append({'K-Fold': fold, 'Akurasi': accuracy, 'Presisi': precision, 'Recall': recall, 'F1-Score': f1})
...
def fold_no(a, b, n):
    ...
    for fold, (train_index, test_index) in enumerate(kf.split(data[0]), 1):
        ...
        recall = round(recall_score(y_test_fold, y_pred_fold, average='weighted', zero_division=1) * 100, 2)
        f1 = round(f1_score(y_test_fold, y_pred_fold, average='weighted', zero_division=1) * 100, 2)
        results_gs.append({'K-Fold': fold, 'Akurasi': accuracy, 'Presisi': precision, 'Recall': recall, 'F1-Score': f1})
